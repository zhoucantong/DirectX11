...
protected:
	...
	UINT m4xMsaaQuality; // quality level of 4X MSAA
						 // Used to keep track of the "delta-time" and game time (¡ì4.3).
	GameTimer mTimer;
	// The D3D11 device (¡ì4.2.1), the swap chain for page flipping
	// (¡ì4.2.4), the 2D texture for the depth/stencil buffer (¡ì4.2.6),
	// the render target (¡ì4.2.5) and depth/stencil views (¡ì4.2.6), and
	// the viewport (¡ì4.2.8).
	...
	int mClientWidth;
	int mClientHeight;
	// True to use 4X MSAA (¡ì4.1.8). The default is false.
	bool mEnable4xMsaa;
};
